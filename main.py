...
class Model:
    ...
    def _train(self, pre_optimizer, train_optimizer, f_out):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for train in get_train_batch_data(self.train, self.opt.batch_size, self.opt.keep_prob1, self.opt.keep_prob2):
                ...
                loss = loss_emo_pair+ emotion_loss
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    ec_test, e_test, c_test = self._evaluate_acc_f1()
                    print('Test: emotion-caus-pair: P {:.4f} R {:.4f} F {:.4f}\n'.format(ec_test[0], ec_test[1], ec_test[2]))
                    print('Test: emotion: P {:.4f} R {:.4f} F {:.4f}\n'.format(e_test[0], e_test[1], e_test[2]))
                    print('Test: cause: P {:.4f} R {:.4f} F {:.4f}\n'.format(c_test[0], c_test[1], c_test[2]))

                    f_out.write('Test: emotion-caus-pair: P {:.4f} R {:.4f} F {:.4f}\n'.format(ec_test[0], ec_test[1], ec_test[2]))
                    f_out.write('Test: emotion: P {:.4f} R {:.4f} F {:.4f}\n'.format(e_test[0], e_test[1], e_test[2]))
                    f_out.write('Test: cause: P {:.4f} R {:.4f} F {:.4f}\n'.format(c_test[0], c_test[1], c_test[2]))

                    if ec_test[2] > ec_max_test_f1:
                        ec_max_test_f1 = ec_test[2]
                        ec_max_test_pre = ec_test[0]
                        ec_max_test_rec = ec_test[1]

                    if e_test[2] > e_max_test_f1:
                        e_max_test_f1 = e_test[2]
                        e_max_test_pre = e_test[0]
                        e_max_test_rec = e_test[1]

                    if c_test[2] > c_max_test_f1:
                        c_max_test_f1 = c_test[2]
                        c_max_test_pre = c_test[0]
                        c_max_test_rec = c_test[1]
    ...
    def _evaluate_acc_f1(self):
        ...
        with torch.no_grad():
            ...
            ec, e, c = self.eval_func(all_doc_pairs, all_emo_cau_pairs)
            return ec, e, c
    ...
    def run(self, folder, repeats=1):
        ...
        if not os.path.exists('log/without_pre/'):
            os.mkdir('log/without_pre/')

        f_out = open('log/without_pre/' + self.opt.model_name + '_' + str(folder) + '_test.txt', 'a+', encoding='utf-8')
        ...
        for i in range(repeats):
            ...
            self._reset_params()
            ec_max_test, e_max_test, c_max_test = self._train(pre_optimizer, train_optimizer, f_out)
            print('ec_max_test: {}     e_max_test: {}   c_max_test: {}\n'.format(ec_max_test, e_max_test, c_max_test))
            f_out.write('ec_max_test: {}     e_max_test: {}   c_max_test: {}\n'.format(ec_max_test, e_max_test, c_max_test))
